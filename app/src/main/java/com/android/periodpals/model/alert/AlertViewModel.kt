...
import com.android.periodpals.model.authentication.AuthenticationModel
    ...
    fun getAlertsByUser(uid: String): List<Alert>? {
        ...
        viewModelScope.launch {
            alertModelSupabase.getAlertsFilteredBy(
                // ideally the uid would not be passed as argument and instead we could get uid by UserViewModel.currentUser?.uid
                cond = { eq("uid", uid) },
                onSuccess = { alerts ->
                ...
                onFailure = { e ->
                    Log.e(TAG, "getMyAlerts: fail to get alerts: ${e.message}")
                }
            )
        }
        return alertsList
    ...
    fun deleteAlert(alert: Alert){
        viewModelScope.launch {
            alert.id?.let {
                ...
                Log.e(TAG, "deleteAlert: fail to delete alert: id of the Alert is null")
            }
        }
    }
}